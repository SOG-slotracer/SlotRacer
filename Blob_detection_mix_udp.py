...
def calculateDistTravel(init_x,init_y,final_x,final_y,coords):
    ...
    if start_index<final_index:
        for pts in range(start_index+1,final_index+1):
            dist_travelled=dist_travelled+math.sqrt(((coords[index][0]-coords[s][0])*pixelX)**2+((coords[index][1]-coords[s][1])*pixelY)**2)
            s=pts
    elif start_index>final_index:
        for pts in range(start_index+1,l):
            dist_travelled=dist_travelled+math.sqrt(((coords[index][0]-coords[s][0])*pixelX)**2+((coords[index][1]-coords[s][1])*pixelY)**2)
            s=pts
...
def getTrackCoordinates():
    #image=cv2.imread("C:/Users/srrajago/Pictures/Camera Roll/CompleteImage.jpg",0) # add the path to the image or image fro video cam
    image=cv2.imread(r"C:\Users\patluu\Documents\slotRacer\map2.png",0) # add the path to the image or image fro video cam
    if image is None:
